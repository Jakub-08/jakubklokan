---
layout: photogallery_photos
description: Fotogalerie z Mistrovství ČR U17. Zápasy, slavnostní vyhlášení a turnajové momenty Jakuba Klokana během soutěže.
title: "Mistrovství ČR U17 | Jakub Klokan"
backLink: "/Photo_gallery/Gallery_24/Gallery_24.html"
---

<div class="gallery-card photo">
  <picture>
    <img src="../MČR_U17/MČR_U17_-_ceny.jpg"
         fetchpriority="high"
         loading="lazy"
         alt="Slavnostní vyhlášení Mistrovství ČR U17 – Jakub Klokan" />
  </picture>
</div>

<div class="gallery-card photo">
  <picture>
    <img src="../MČR_U17/MČR_U17_-_single.jpg"
         fetchpriority="high"
         loading="lazy"
         class="img-top"
         alt="Singlový zápas na Mistrovství ČR U17 – Jakub Klokan" />
  </picture>
</div>